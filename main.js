import { svgEdit, svgDelete, svgTel, svgVK, svgMail, svgFB, svgOther, addClient, svgDeleteContact, } from './svg.js'
import { modalClientsDel } from './modal-del.js'


const SERVER_URL = 'https://clients-maks0n.amvera.io'
...
const modalWindow = (open, close) => {
  ...
  open(modalTitle, modalContent, modalCloseBtn, inpSurname, inpName, inpLastname, modalContacts, addBtn, addForm)

  document.body.append(modal)
  modal.append(modalContainer)
  modalContainer.append(closeBtnX, modalTitleBlock, modalTitle, modalContent, modalCloseBtn)
  modalTitleBlock.append(modalTitle)
  modalContent.append(addForm)
  addForm.append(labelSurname, inpSurname, labelName, inpName, labelLastname, inpLastname, modalContacts, validation, addBtn)
...
function getUpperCase(inp) {
  return inp.value.trim().substring(0, 1).toUpperCase() + inp.value.trim().substring(1).toLowerCase()
}
...
idTh.addEventListener('click', () => {
  sortCol = 'id'
  sortDir = !sortDir
  sortDir ? idTh.innerHTML = 'id <span> 🡣 </span>' : idTh.innerHTML = 'id <span> 🡡 </span>'
  render(clients)
})
...
fioTh.addEventListener('click', () => {
  sortCol = 'fio'
  sortDir = !sortDir
  sortDir ? fioTh.innerHTML = 'Фамилия Имя Отчество <span> 🡣 А-Я </span>' : fioTh.innerHTML = 'Фамилия Имя Отчество <span> 🡡 Я-А </span>'
  render(clients)
})
...
сreatedTh.addEventListener('click', () => {
  sortCol = 'createdAt'
  sortDir = !sortDir
  sortDir ? сreatedTh.innerHTML = 'Дата и время создания <span> 🡣 </span>' : сreatedTh.innerHTML = 'Дата и время создания <span> 🡡 </span>'
  render(clients)
})
...
updatedTh.addEventListener('click', () => {
  sortCol = 'updatedAt'
  sortDir = !sortDir
  sortDir ? updatedTh.innerHTML = 'Последние изменения <span> 🡣 </span>' : updatedTh.innerHTML = 'Последние изменения <span> 🡡 </span>'
  render(clients)
})
...
const filterForm = document.getElementById('header-form').addEventListener('submit', (e) => {
  e.preventDefault()
...
const formClean = document.getElementById('form-clean').addEventListener('click', () => {
  inpFilter.value = ''
  render(clients)
})
...
function getClientItem(clientObj) {
  ...
  const tableDelete = createEl('td', 'table__cell', 'table-delete')
  ...
  btnBurger.addEventListener('click', () => {
    let contactsHidden = document.querySelectorAll('.table__cell_contacts-hidden')
  ...
  tableBodyTr.append(tableId, tableFio, tableCreated, tableUpdated, tableContacts, tableEdit)
  tableCreated.append(createdTime)
  tableUpdated.append(updatedTime)
  tableEdit.append(btnEdit, btnDelete)
  /* tableDelete.append(btnDelete) */
  ...
  btnEdit.addEventListener('click', () => {
    modalWindow((modalTitle, modalContent, modalCloseBtn, inpSurname, inpName, inpLastname, modalContacts, addBtn, addForm) => {
      ...
      for (let contact of clientObj.contacts) {
        ...
        contactEdit.contact.firstChild.firstChild.lastChild.firstChild.textContent = contact.type
      ...
      addForm.addEventListener('submit', async (e) => {
        ...
        let contactsTypes = document.querySelectorAll('.choices__list--single')
        let contactsValues = document.querySelectorAll('.modal__contacts-inp')
        ...
        await serverPatch(clientObj.id, inpName.value, inpLastname.value, inpSurname.value)
        render(clients)
      })
...
async function render(copyArray) {
  ...
  if (serverData !== '' && serverData !== null) {
    copyClients = serverData.map((client) => { client.createdAt = new Date(client.createdAt); return client })
    copyClients = serverData.map((client) => { client.updatedAt = new Date(client.updatedAt); return client })
  }

  for (let client of copyClients) {
    client.fio = client.surname + ' ' + client.name + ' ' + ' ' + client.lastName
    client.filter = client.surname.substring(0).toLowerCase() + ' ' + client.name.substring(0).toLowerCase() + ' ' + client.lastName.substring(0).toLowerCase()
